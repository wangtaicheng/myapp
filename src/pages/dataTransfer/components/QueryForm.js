...
import {
  Form, Button, Input, Select, Row, Col,DatePicker,
} from 'antd';
        ...
        <Col ms={24} md={8} lg={8}>
          <FormItem {...formItemLayout} label="驾驶年龄">
            ...
            {getFieldDecorator('drivingAge', {
              rules: [{required: true, message: '驾龄不能为空'}, {
                max: 2,
                pattern: '^[0-9]*$',
                message: '只能是数字且不能超过2位数'
              }]
            })(
      ...
      <Row>
        <Col ms={24} md={8} lg={8}>
          <FormItem {...formItemLayout} label="时间">
            {/* initialValue是默认值，rules一条规则，一个对象  */}
            {getFieldDecorator('drivingDate')(
              <MonthPicker onChange={onChange} placeholder="Select month" />,
            )
            }
          </FormItem>
        </Col>
        <Col  ms={24} md={8} lg={8} style={{textAlign: 'right'}}>
